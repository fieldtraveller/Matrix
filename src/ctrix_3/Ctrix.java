...
public class Ctrix {	//CLASS NAME
	...
	public Ctrix(int Number_of_Rows,int Number_of_Columns,int ...Elements)	//CONSTRUCTOR
	{
		...
		this.Number_of_Rows=Number_of_Rows;	//SET VALUE BASED ON INPUT
		this.Number_of_Columns=Number_of_Columns;	
		this.Elements=new double[Number_of_Rows][Number_of_Columns];	//INITIALISE THE 2D ARRAY
		
		try{
			if((this.Number_of_Rows*this.Number_of_Columns)>(Elements.length))	//IF TOO FEW PARAMETERS
			{
				throw new CtrixException("Not Enough Parameter/Arguments.");	//THROW EXCEPTION
			}
			if((this.Number_of_Rows*this.Number_of_Columns)<(Elements.length))	//IF TOO MANY PARAMETERS
			{
				throw new CtrixException("Too Many Parameter/Arguments.");	//THROW EXCEPTION
			}
			else
			{
				int count_2=0;
				for(int count_0=0;count_0<this.Number_of_Rows;count_0++)
				{
					...
					{
						this.Elements[count_0][count_1]=Elements[count_2];	//SET VALUE BASED ON INPUT
						count_2++;
					}
				}
			}
	...
	public Ctrix CallBack(CallBackInterface InterfaceInstance)	//PERFORM CALLBACK OF FUNCTION OF THE ELEMENTS OF THE MATRIX
	{
		...
		for(int count_0=0;count_0<this.Number_of_Rows;count_0++)
		{
			for(int count_1=0;count_1<this.Number_of_Columns;count_1++)
			{
				return_value[count_0][count_1]=InterfaceInstance.CallBackMethod(this.Elements[count_0][count_1]);	//CALL BACK
			}
		}
	...
	public double Det()	//COMPUTE THE DETERMINANT OF THE MATRIX
	{
		...
		else if(this.Number_of_Columns == 2)	//IF MATRIX IS A 2x2 SQUARE MATRIX
		{
